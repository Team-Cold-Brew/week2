...
<head>
    ...
    <title>MobileFix - Actualizar Orden</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        <a href="<c:url value='/tech/dashboard'/>" class="btn-back">← Volver al Dashboard</a>
        ...
        <div class="order-card">
            ...
            <div class="info-grid">
                ...
                <div class="info-item">
                    ...
                    <div class="info-value">
                        <c:choose>
                            <c:when test="${order.updatedAt != null}">
                                <fmt:formatDate value="${order.updatedAt}" pattern="dd/MM/yyyy HH:mm"/>
                            </c:when>
                            <c:otherwise>
                                <span style="color: #999;">Sin actualizaciones</span>
                            </c:otherwise>
                        </c:choose>
                    </div>
                </div>
            ...
            <c:if test="${order.status != 'DELIVERED' && order.status != 'CANCELED'}">
                <div class="update-form">
                    <h3 class="section-title"> Actualizar Estado de la Orden</h3>

                    <div class="status-info">
                         <strong>Transiciones permitidas:</strong><br>
                        PENDING → IN_PROGRESS o CANCELED<br>
                        IN_PROGRESS → READY o CANCELED<br>
                        READY → DELIVERED
                    </div>
                    ...
                    <form id="updateOrderForm">
                        ...
                        <div class="form-group">
                            <label for="techNotes">Notas Técnicas:</label>
                            <textarea id="techNotes"
                                      name="techNotes"
                                      placeholder="Describe el trabajo realizado, piezas reemplazadas, etc...">${order.techNotes}</textarea>
                        </div>
            ...
            <c:if test="${order.status == 'DELIVERED' || order.status == 'CANCELED'}">
                <div class="status-info">
                     Esta orden ya ha sido ${order.status == 'DELIVERED' ? 'entregada' : 'cancelada'} y no puede ser modificada.
                </div>
            </c:if>
        </div>
    </div>
