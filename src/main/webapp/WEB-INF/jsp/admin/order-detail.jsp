...
<head>
    ...
    <title>MobileFix - Gestionar Orden</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    ...
    <div class="container">
        ...
        <div class="order-card">
            ...
            <c:choose>
                <c:when test="${order.techNotes != null && !order.techNotes.isEmpty()}">
                    <div class="tech-notes">
                        ${order.techNotes}
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="description-box no-notes">
                        Aún no hay notas del técnico para esta orden
                    </div>
                </c:otherwise>
            </c:choose>
            ...
            <div class="admin-actions">
                <h3 class="section-title">⚙️ Acciones de Administrador</h3>

                <!-- Asignar Técnico -->
                <c:if test="${order.status != 'DELIVERED' && order.status != 'CANCELED'}">
                    <form id="assignTechForm">
                        <div class="form-group">
                            <label for="techId">Asignar/Cambiar Técnico:</label>
                            <select id="techId" name="techId" required>
                                <option value="">Selecciona un técnico</option>
                                <c:forEach items="${technicians}" var="tech">
                                    <option value="${tech.id}"
                                            ${order.assignedTech != null && order.assignedTech.id == tech.id ? 'selected' : ''}>
                                        ${tech.fullName} (${tech.username})
                                    </option>
                                </c:forEach>
                            </select>
                        </div>
                        <button type="submit" class="btn-primary">Asignar Técnico</button>
                    </form>
                    <hr style="margin: 20px 0; border: none; border-top: 1px solid #e0e0e0;">
                </c:if>

                <!-- Eliminar Orden -->
                <button onclick="deleteOrder(${order.id})" class="btn-danger">
                    🗑️ Eliminar Orden
                </button>
            </div>
        </div>
    ...
    <script>
        // Asignar técnico
        document.getElementById('assignTechForm')?.addEventListener('submit', async (e) => {
            e.preventDefault();
            ...
            try {
                const response = await fetch('/api/orders/${order.id}/assign/' + techId, {
                    method: 'PUT'
                });
